...
public class PrayerSwitchTask extends Task<ClientContext> {
	private final int[] attackIds = { 24047, // range attack
			24095 // mage attack
	};
	...
	public boolean activate() {
		...
		if (!ctx.projectiles.select().id(this.mageProjectileAraxxorId)
				.isEmpty()) { /* There's a mage projectile somewhere */
			Iterator<Projectile> iterator = ctx.projectiles.select()
					.id(this.mageProjectileAraxxorId).iterator();
			while (iterator.hasNext()) {
				...
				if (closestMageProjectile == null) {
					...
					if (ctx.movement.distance(projectile) < ctx.movement
							.distance(closestMageProjectile)) {
						closestMageProjectile = projectile;
					}
				}
			}
		...
		if (!ctx.projectiles.select().id(rangeProjectileAraxxorId).isEmpty()) {
			/*
			 * There 's a range projectile somewhere
			 */
			Iterator<Projectile> iterator = ctx.projectiles.select()
					.id(this.rangeProjectileAraxxorId).iterator();
			while (iterator.hasNext()) {
				Projectile projectile = iterator.next();
				if (closestRangeProjectile == null) {
					...
					if (ctx.movement.distance(projectile) < ctx.movement
							.distance(closestRangeProjectile)) {
						closestRangeProjectile = projectile;
					}
				}
			}
		...
		/*
		 * At this point we have the closest range projectile and the closest
		 * mage projectile to the player
		 */
		if (closestMageProjectile != null) {
			if (closestRangeProjectile != null) {
				/*
				 * There's both a range and mage projectile
				 */
				if (ctx.movement.distance(closestMageProjectile) < ctx.movement
						.distance(closestRangeProjectile)) {
		...
		} else if (closestRangeProjectile != null) {
			/*
			 * If we get here it means we have no mage projectile but we do have
			 * a range projectile
			 */
			return !this.isPlayerPrayingRange();
	...
	public void execute() {
		...
		/*
		 * Sleep a little bit so as to account for any delays between pressing
		 * the key and having the prayer icon change
		 */
		try {
